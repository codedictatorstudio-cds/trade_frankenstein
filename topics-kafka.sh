...
# Helper to create topics with multiple --config flags
create_topic() {
  local name="$1"; local parts="$2"; local repl="$3"; shift 3
  docker exec kafka-broker kafka-topics.sh     --bootstrap-server "$BROKER"     --create --if-not-exists     --topic "$name"     --partitions "$parts"     --replication-factor "$repl"     "$@"
}
